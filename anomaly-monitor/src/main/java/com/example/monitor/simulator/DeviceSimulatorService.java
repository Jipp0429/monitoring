 ...
 * 평소에는 정상 범위(평균 50, 표준편차 5)에서 값이 나오지만,
 * 확률적으로(ANOMALY_PROBABILITY) 급격히 튀는 이상치를 섞어 넣어
...
public class DeviceSimulatorService {
    ...
    private static final double BASELINE_STDDEV = 5.0;
    private static final double ANOMALY_PROBABILITY = 0.03; // 3% 확률로 이상치 주입
    private static final double ANOMALY_MAGNITUDE = 40.0;
    ...
    private final AtomicInteger deviceCount = new AtomicInteger(100);
    ...
    public void tick() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            double value = BASELINE_MEAN + random.nextGaussian() * BASELINE_STDDEV;

            if (random.nextDouble() < ANOMALY_PROBABILITY) {
                double direction = random.nextBoolean() ? 1 : -1;
                value += direction * ANOMALY_MAGNITUDE * random.nextDouble();
            }

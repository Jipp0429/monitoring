...
- 최종보고서(hwp, 이미 제출됨)의 수치(100.1 → 974.9 → 4,975.7 events/sec 등)와 논문 초안의 수치는 **더 이상 통일하지 않기로 결정함(2026-09-11 정정)**. 논문은 재측정값(104.2 → 1,026.3 → 5,001.2 events/sec)으로 정직하게 업데이트하고, 왜 이미 제출된 최종보고서와 논문 수치가 달라졌는지 설명하는 문장을 논문에 넣기로 함. 자세한 내용은 아래 "알려진 이슈" 참고.
...
- **[해야 할 일] `docs/paper_draft.md`/`.docx`가 아직 옛날 수치(100.1 → 974.9 → 4,975.7 등, 제출된 최종보고서와 맞춰뒀던 값)로 남아있음.**
  **결정이 바뀌어서(2026-09-11)** 이제는 재측정값(104.2 → 1,026.3 → 5,001.2 events/sec)으로 논문을 업데이트하기로 했고,
  최종보고서(hwp, 이미 제출됨)와 논문 수치가 달라지는 이유 — 제출 이후 추가 안정성 검증 중 `directBestEffort` 회귀를
  발견·수정했고, 그 재검증 과정에서 측정 방식 버그까지 고쳐 더 정확한 수치를 얻었다는 것 — 를 설명하는 문장을 논문에
  넣기로 함. **아직 실제로 paper_draft.md/.docx 파일 수정은 안 된 상태 — 다음에 이어서 해야 함.**
